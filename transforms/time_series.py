...
def remove_frequency(x, pertub_ratio=0.0):
    mask = (
        torch.FloatTensor(x.shape).uniform_() > pertub_ratio
...
def masking(x, keepratio=0.9, mask="binomial"):
    ...
    if mask == "binomial":
        mask_id = generate_binomial_mask(
            x.size(0), x.size(1), x.size(2), p=keepratio
        ).to(x.device)
...
def DataTransform_TD(sample, ratio = 0.8):
    """Simplely use the jittering augmentation.
    Feel free to add more autmentations you want, but we noticed that in TF-C framework,
    the augmentation has litter impact on the final tranfering performance.
    """
    aug = jitter(sample, ratio)
    return aug
...
def DataTransform_TD_bank(sample, config):
    ...
    aug_1 = (
        aug_1 * li_onehot[:, 0][:, None, None]
    )  # the rows that are not selected are set as zero.
